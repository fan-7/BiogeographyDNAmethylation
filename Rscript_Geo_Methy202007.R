...
dev.off()
load("//geography/190809Geography/usedAno.RData")
gc()
...
load("//geography/190809Geography/usedMethy.RData")
...
meanAUC <- meanAUC[-1]
png("//ancestry/try5_intersect3qc/multilogit/ancestry.y3Minor.10snp.AUC.png")
...
write.table(y3select10SNP, file= "//ancestry/try5_intersect3qc/multilogit/y3.minor.select.SNP", quote = FALSE, row.names = FALSE)
###############################
	  ...
	  ewasFun <- function(usenormbetadata = FALSE, phedata, selphe, covariates = c("B","NK","CD4T","CD8T","Mono","Neutro","Eosino","PC1","PC2","PC3","PC4","PC5"), type = "linear", isbetadependent = FALSE, savepath = ".", logitmaxit = 25, chunknum = 1, plotmq = TRUE)
{
    ...
    defcov <- read.csv("/liuxx/DNAm_data/defcov.csv", as.is = TRUE) #default covariates
    ...
    load("/Group/liuxx/DNAm_data/genoinfo.RData") #genomic information
    if(usenormbetadata){
        load("/liuxx/DNAm_data/norm_order_betadata.RData") #normalized and ordered beta values data
        betadata <- norm_order_betadata
        rm(norm_order_betadata)
        gcresult <- gc()
    } else{
        load("/liuxx/DNAm_data/betadata.RData") #primiry beta values data
    }
...
for(i in 1: 20){
    if(i ==1){
	   ...
	   load("/liuxx/DNAm_data/norm_order_betadata.RData")
